...
async function deleteFromDB(id) {
  ...
  let final = elements.filter(item => {
    console.log(item)
    console.log(id)
    if (item.id !== id) {
      return item;
  ...
  console.log(final);
  localStorage.setItem('diego-items', JSON.stringify(final));
}

async function resetDB(list) {
  list = [{
    id: 0,
    link: "www.notion.so",
    img: "notion.png",
  ...
  {
    id: 1,
    link: "github.com/Diego-Lopes-Ferreira",
    img: "github.png",
    title: "Github",
...
function createCard(item) {
  ...
  cardA.setAttribute("href", `https://${link}`);
  cardA.setAttribute("target", "blank");
  ...
  cardA.appendChild(h1);
  cardA.appendChild(paragraph);
  //card.appendChild(btn);
  card.appendChild(cardA);
  mainPart.appendChild(card);
}

// Render A lot of Cards
...
async function generateQuote() {
  ...
  let complete = movieQuotes[number];
  let last = movieQuotes[number].lastIndexOf('\" ');
